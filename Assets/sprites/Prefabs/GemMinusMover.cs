...
public class GemMinusMover : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D other) //other chính là thông tin của bất kỳ collider nào va chạm với collider này
    {
        ...
        {
            Debug.Log("viên ngọc đỏ đã va chạm với game object có nhãn player");
        ...
        {
            Debug.Log("viên ngọc đỏ đã va chạm với game object có nhãn ground");
            Destroy(gameObject);
            Debug.Log("đã xóa viên ngọc này");
        }
